...
if (typeof require === "undefined") {
    var require = function (pk) {
        if (pk === "ul") {
            return {
                merge: function () { var dst = {} , src , p , args = [].splice.call(arguments, 0) ; while (args.length > 0) { src = args.splice(-1)[0]; if (toString.call(src) != "[object Object]") { continue; } for (p in src) { if (!src.hasOwnProperty(p)) { continue; } if (toString.call(src[p]) == "[object Object]") { dst[p] = this.merge(src[p], dst[p] || {}); } else { if (src[p] !== undefined) { dst[p] = src[p]; } }; } } return dst; }
            };
        }
        if (pk === "querystring") {
            return {
